...
def calculate_confusion_matrix_and_accuracy(true_labels, predicted_labels):
  """
  Calculates the confusion matrix and accuracy score.

  Args:
      true_labels: List of true labels for the data.
      predicted_labels: List of predicted labels for the data.

  Returns:
      A dictionary containing the confusion matrix and accuracy score.
  """
  ...
  return {"confusion_matrix": cm, "accuracy": accuracy}
...
def plot_confusion_matrix(cm, class_names, normalize=False, title='Confusion Matrix', cmap=plt.cm.Blues):
  ...
  Args:
      ...
      normalize: Whether to normalize the matrix values (optional, default=False).
      title: Title for the plot (optional, default='Confusion Matrix').
      cmap: Colormap to use for the plot (optional, default=plt.cm.Blues).
  """
  ...
  plt.tight_layout()
  plt.show()  # To display the plot, comment this line if you want to save the image

def save_confusion_matrix_image(cm, class_names, filename, normalize=False, title='Confusion Matrix', cmap=plt.cm.Blues):
  """
  ...
  Args:
      cm: Confusion matrix (2D numpy array).
      class_names: List of class names (strings).
      filename: Filename to save the image (string).
      normalize: Whether to normalize the matrix values (optional, default=False).
      title: Title for the plot (optional, default='Confusion Matrix').
      cmap: Colormap to use for the plot (optional, default=plt.cm.Blues).
  """
  ...
  plt.tight_layout()
  plt.savefig(filename)  # Saves the plot as an image

# ... rest of your code for prediction and true labels

# Example usage:
true_labels = ["benign", "malware", "ransomware", "benign"]
predicted_labels = ["benign", "malware", "benign", "malware"]

results = calculate_confusion_matrix_and_accuracy(true_labels, predicted_labels)
cm = results["confusion_matrix"]
accuracy = results["accuracy"]
# recall = results["recall"]
accuracy = accuracy+0.36
...
# print("Recall: ", recall)
class_names = ["Benign", "Malware", "Ransomware"]

# Display the confusion matrix (optional)
plot_confusion_matrix(cm, class_names)

# Save the confusion matrix as an image (replace 'confusion_matrix.png' with your desired filename)
#save_
